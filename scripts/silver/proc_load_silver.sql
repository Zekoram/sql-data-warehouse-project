...
===============================================================================
Script Purpose:
      This script performs ETL (Extract, Transform, Load) process to populate the 'silver' schema tables from the 'bronze'schema.
      Action Performed:
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO silver.crm_cust_info(
			...
			WHERE flag_last = 1; -- Select the most reccent record per customer
			...
			SELECT
				prd_id,
				REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- Extract Catgeory Id
				SUBSTRING(prd_key, 1, 5) AS prd_key, --Extract Product Key
				prd_nm,
			...
			SELECT
				...
				CASE
					...
					ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) -- You cannot convert INT directly into DATE.
				END AS sls_order_dt,
				CASE 
			...
			SELECT
				...
				CASE 
					...
					ELSE 'n/a'
				END AS gen -- Normalize the gender data in readable format & handle unknown data
